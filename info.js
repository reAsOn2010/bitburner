...
export async function main(ns) {
    ...
    for (const h of hosts) {
        let _info = {
            ...
            "hack_time": ns.getHackTime(h) / 3600 / 1000,
            "grow_time": ns.getGrowTime(h) / 3600 / 1000,
            "weaken_time": ns.getWeakenTime(h) / 3600 / 1000,
        }
        if (_info["root"]) {
            info[h] = _info
        }
    ...
    for (const _info of Object.values(info)) {
        ns.tprintf("%s|%f|%f|%f", 
            _info["name"], _info["hack_time"].toFixed(2), _info["grow_time"].toFixed(2), _info["weaken_time"].toFixed(2))
    }
}